...
function MainScreen({
  ...
  const renderFriendsScreen = () => (
    <div className="lcd-content">
      ...
      {/* Friend Requests Section - More Prominent */}
      {friendRequests.length > 0 && (
        <div style={{ marginBottom: '8px' }}>
          ...
          {friendRequests.map((request, index) => (
            ...
            >
              <span style={{ color: '#ffaa00', fontWeight: 'bold' }}>
                {request.username?.toUpperCase() || 'UNKNOWN'}
              </span>
          ...
          <div className="status-line" style={{ margin: '6px auto' }}></div>
        </div>
      )}

      {/* Add Friend Section - Compact */}
      <div style={{ 
        display: 'flex', 
        gap: '4px', 
        marginBottom: '8px', 
        ...
        <button
          ...
          style={{
            background: newFriendUsername.trim() ? '#006600' : '#333',
            color: newFriendUsername.trim() ? '#00ff44' : '#666',
            border: 'none',
            padding: '3px 8px',
            borderRadius: '2px',
            fontSize: 'clamp(9px, 2.5vw, 12px)',
            cursor: newFriendUsername.trim() ? 'pointer' : 'not-allowed'
          }}
        >
          ADD
        </button>
      ...
      {friends.length === 0 ? (
        ...
        <div>
          <div className="lcd-text" style={{ 
            ...
            {friends.length} CONTACT{friends.length > 1 ? 'S' : ''} • TAP TO CALL
          </div>
          {friends.map((friend) => (
            <div 
              key={friend.id}
              ...
              onMouseEnter={(e) => {
                e.target.style.backgroundColor = 'rgba(0, 255, 68, 0.15)'
                e.target.style.borderColor = 'rgba(0, 255, 68, 0.4)'
              }}
              onMouseLeave={(e) => {
                e.target.style.backgroundColor = 'rgba(0, 255, 68, 0.05)'
                e.target.style.borderColor = 'rgba(0, 255, 68, 0.2)'
              }}
            >
              <span style={{ 
                color: '#00ff44', 
